...
describe('USERS TEST', () => {
  ...
  test('renders components', async () => {
    // сейчас в переменной response у нас есть схема данных, которую нам возвращает бэк. Необходимо согласовать эту схему с методом гет аксиоса.делаем запрос
  ...
  test('redirect to details page', async () => {
    axios.get.mockReturnValue(response);
    // render(
    //   // если оставить так, то тест будет падать в ошибку, т.к. мы пытаемся перейти на роут, которого нет в компоненте юзерс (а именно на userDetailPage). Поэтому нам нужно добавить роуты, к которым мы хотим обратиться
    //   // <MemoryRouter>
    //   //   <Users />
    //   // </MemoryRouter>
